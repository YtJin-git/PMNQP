...
class Evaluator:
    ...
    def score_manifold_model(self, scores, obj_truth, bias=0.0, topk=1, config=None):
        '''
...
def predict_logits(model, dataset, config):
    ...
    pairs = torch.tensor([(attr2idx[attr], obj2idx[obj])
                                for attr, obj in pairs_dataset]).to(model.device)
...
def test(
    ...
    results = evaluator.score_model(
        all_pred_dict, all_obj_gt, bias=1e3, topk=1, config=config
    )
