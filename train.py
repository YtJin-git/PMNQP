...
def train_model(model, optimizer, config, train_dataset, val_dataset, test_dataset):
    ...
    train_pairs = torch.tensor([(attr2idx[attr], obj2idx[obj])
                                for attr, obj in train_dataset.train_pairs]).to(model.device)
    ...
    for i in range(config.epoch_start, config.epochs):
        ...
        val_flag = True
        if val_flag:
            print("--- Evaluating validation dataset on Closed World ---")
...
if __name__ == "__main__":
    ...
    model = get_model(config, attributes=attributes, classes=classes, offset=offset, dset=train_dataset).to(device)
